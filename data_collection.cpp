...
const std::vector<int> sensorChannels = {0x02, 0x04, 0x08, 0x10, 0x20};
const std::vector<int> sensorChannelsBackup = {1, 2, 3, 4, 5};
const std::vector<std::string> sensorLocations = {"Left Knee", "Left Hip", "Right Ankle", "Right Knee", "Right Hip"};
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < sensorChannels.size(); i++) {
        ...
        usleep(10000);
        sensors[i].bus_write = I2C_bus_write_with_tca;
        sensors[i].bus_read = I2C_bus_read;
        sensors[i].delay_msec = delay_msec;
        sensors[i].dev_addr = 0x29;
        if (bno055_init(&sensors[i]) != BNO055_SUCCESS || bno055_set_operation_mode(BNO055_OPERATION_MODE_NDOF) != BNO055_SUCCESS) {
            std::cerr << "Failed to initialize sensor at " << sensorLocations[i] << std::endl;
        }
    }
