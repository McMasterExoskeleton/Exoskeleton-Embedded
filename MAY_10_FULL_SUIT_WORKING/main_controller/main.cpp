...
const int MOTOR_NUMBER = 0;   // number of motors 0-4, THIS IS NOT USED DOUBLE CHECK WHAT THIS IS FOR 
...
int main() {
  ...
  while (true) {
    ...
    for (size_t i = 0; i < torque_values.size(); ++i) {
      torque_values[i] = clampTorque(torque_values[i], -250, 250);  // Clamp to [-250, 250]
      torque_values[i] = limitTorqueChange(torque_values[i], previous_torque_values[i], 100);  // Limit change to 50, max toruqe change per time interval
    }
