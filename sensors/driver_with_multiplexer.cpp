...
s8 I2C_bus_write_with_tca(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt) {
        // First, select the desired channel on the multiplexer.
    // Assuming you have a global or accessible instance of rpi_tca9548a named 'mux'
    tca.set_channel(0x02);
    
    // Optionally, you may want to add a small delay to allow the multiplexer to switch:
    usleep(1000); // 10 ms delay (adjust if necessary)

    // Now proceed with the normal I2C write.
...
int main() {
   ...
   // if (wiringPiI2CSetup() == -1) {
    ...
    bno055.bus_write = I2C_bus_write_with_tca;
